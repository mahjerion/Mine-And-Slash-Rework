...
public class MnsRelicAffixes extends ExileKeyHolder<RelicAffix> {
    ...
    // the main mod has no relic type of its own any more: these loot affixes roll on dungeon relics, the
    // only kind the map device takes. Old relics with type "mmorpg" are relabelled on load, see
    // DungeonItemNbt.RELIC.
    static String TYPE = DungeonMain.MODID;

    // implicits are bound to dungeon relics too - a map content guarantee only means something on a relic
    // that can actually enter the map device
    static String IMPLICIT_TYPE = DungeonMain.MODID;
    ...
    // Implicits, one of which every dungeon relic rolls into its dedicated implicit slot. Flat 100 so the
    // guarantee doesn't depend on the affix roll.

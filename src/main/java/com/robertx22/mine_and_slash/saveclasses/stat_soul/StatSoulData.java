...
public class StatSoulData implements ICommonDataItem<GearRarity>, ISettableLevelTier {
    ...
    public ExplainedResult canInsertIntoStack(ItemStack stack) {
        ...
        // these items are generated by the datapack into a specific unique, souling them was the way
        // players bypassed that gate and got a common version of an item meant to be unique only
        if (!AutoItem.getAllFor(stack.getItem()).isEmpty()) {
            return ExplainedResult.failure(Chats.NOT_GEAR_OR_NOT_COMPAT.locName());
        }
